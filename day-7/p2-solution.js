...
const getHandValue = (cards) => {
  ...
  switch (map.size) {
    case 5:
      value = j ? handTypes.PAIR : handTypes.HIGHEST;
      break;
    case 4:
      value = j ? handTypes.THREE_OF_A_KIND : handTypes.PAIR;
      break;
    case 3:
      if (cardOccurences.includes(2)) {
        if (j === 1) {
          value = handTypes.FULL_HOUSE;
        } else if (j === 2) {
          value = handTypes.FOUR_OF_A_KIND;
        } else value = handTypes.TWO_PAIR;
      } else {
        if (j === 1) {
          value = handTypes.FOUR_OF_A_KIND;
        } else if (j === 3) {
          value = handTypes.FOUR_OF_A_KIND;
        } else value = handTypes.THREE_OF_A_KIND;
      }
      break;
    case 2:
      if (cardOccurences[0] === 2 || cardOccurences[0] === 3) {
        value = j ? handTypes.FIVE_OF_A_KIND : handTypes.FULL_HOUSE;
      } else {
        value = j ? handTypes.FIVE_OF_A_KIND : handTypes.FOUR_OF_A_KIND;
      }
      break;
    case 1:
